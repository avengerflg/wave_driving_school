...
<div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="card dashboard-card welcome-card">
                <div class="d-flex align-items-center row m-0">
                    <div class="col-sm-7 col-12 p-4">
                        ...
                        <p class="mb-4">
                            You have <span class="fw-bold">{{ $stats['upcomingBookings'] }}</span> upcoming lessons.<br>
                            @if($stats['nextLessonDate'])
                                Next lesson: <span class="fw-bold">{{ $stats['nextLessonDate'] }}</span>
                            @else
                                Ready to book your next lesson?
                            @endif
                        </p>
                        <div class="d-flex gap-3">
                            <a href="{{ route('booking.index') }}" class="btn btn-light">
                                <i class="bx bx-calendar-plus me-2"></i>Book a Lesson
                            </a>
                            <a href="{{ route('client.bookings.index') }}" class="btn btn-outline-light">
                                <i class="bx bx-calendar me-2"></i>My Bookings
                            </a>
                        </div>
                    </div>
    ...
    <div class="row mb-4">
        @foreach([
            ['title' => 'Total Lessons', 'value' => $stats['totalBookings'], 'icon' => 'bx-calendar-check', 'color' => 'primary'],
            ['title' => 'Completed', 'value' => $stats['completedBookings'], 'icon' => 'bx-check-circle', 'color' => 'success'],
            ['title' => 'Hours Driven', 'value' => $stats['totalHours'], 'icon' => 'bx-time-five', 'color' => 'info'],
            ['title' => 'Upcoming', 'value' => $stats['upcomingBookings'], 'icon' => 'bx-calendar-event', 'color' => 'warning']
        ] as $stat)
        <div class="col-sm-6 col-lg-3 mb-4">
            <div class="card dashboard-card stats-card {{ $stat['color'] }} h-100">
                <div class="card-body">
                ...
                <div class="card-body p-0">
                    <div class="p-3">
                        ...
                        <div class="schedule-item mb-3">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="flex-grow-1">
                                    <h6 class="mb-0">{{ $booking->service->name ?? 'Driving Lesson' }}</h6>
                        ...
                        <div class="text-center py-5">
                            <i class="bx bx-calendar-x text-muted mb-3" style="font-size: 3rem;"></i>
                            <h6 class="text-muted mb-3">No upcoming lessons</h6>
    ...
    <div class="row">
        ...
        <div class="col-12 col-md-8 mb-4">
            <div class="card dashboard-card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">Recent Lessons</h5>
                        <a href="{{ route('client.bookings.index') }}" class="btn btn-sm btn-outline-primary">
                            View All
                        </a>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <div class="timeline-item">
                        <span class="timeline-point 
                            @if($booking->status === 'completed') success
                            @elseif($booking->status === 'cancelled') danger
                            @else primary
                            @endif"></span>
                        <div class="timeline-event">
                            ...
                            <p class="text-muted mb-0">
                                Status: <span class="badge bg-label-
                                    @if($booking->status === 'completed') success
                                    @elseif($booking->status === 'cancelled') danger
                                    @else primary
                    ...
                    <div class="text-center py-4">
                        <i class="bx bx-history text-muted mb-3" style="font-size: 3rem;"></i>
                ...
                <div class="card-body">
                    <div class="d-grid gap-3">
                        <a href="{{ route('booking.index') }}" class="btn btn-primary">
                            <i class="bx bx-calendar-plus me-2"></i>Book New Lesson
                        </a>
...
const progressChart = new ApexCharts(document.querySelector("#progressChart"), {
    series: [{
        ...
        type: 'area',
        data: {{ json_encode($chartData['lessonsCompleted']) }}
    }, {
        name: 'Hours Driven',
        type: 'line',
        data: {{ json_encode($chartData['hoursDriven']) }}
    }],
    chart: {
        height: 300,
        type: 'line',
    ...
    tooltip: {
        ...
        style: { fontSize: '12px' },
        y: [{
            formatter: value => Math.round(value) + ' lessons'
        }, {
